import { computed, defineComponent, FunctionalComponent, h, ref, resolveComponent, unref } from 'vue'
import { ElForm, Column, ElCheckbox, CheckboxValueType, ElTableV2, FormItemRule } from 'element-plus'
import { SelectionCellProps } from '../../types/table'
...
export default defineComponent({
  ...
  setup(props, { slots, attrs, expose }) {
     ...
     * @returns 表头内容
     */
    const renderHeaderCell = (columnItem: Column, scope: any) => {
     ...
     * @returns 选择列内容
     */
    const renderSelection = (scope: any, part: 'header' | 'default') => {
    ...
    const transferColumns = (columns: Column[]) => {
      return columns.map((column) => {
        ...
        column.headerCellRenderer = (scope) => renderHeaderCell(column, scope)
        column.cellRenderer = (scope) => renderDefaultCell(column, scope)
        return column
      })
    }
